...
# -*- coding: utf-8 -*-
"""
支持协议: VMess, VLESS (含Reality), Trojan, SS, SSR, Hysteria2, TUIC, AnyTLS
优化要点: 
  - 严禁跟随重定向 (杜绝劫持假活)
  - 隔离系统环境变量 (杜绝本地代理穿透)
  - 协议自适应内核分流 (VLESS-Reality/Hy2/TUIC 走 Mihomo)
"""
...
# 阶段开关
STAGE1_V2RAY_TEST    = True   # 阶段1: 延迟快速初筛
STAGE1_TOP_N         = 1500   # 阶段1保留前N个节点
STAGE15_REAL_TEST    = True   # 阶段1.5: 真连接Google测试（两轮）
STAGE2_SCHOLAR_TEST  = True   # 阶段2: Scholar学术验证

# 核心路径 (推荐：clash 使用 mihomo，v2ray 可使用 xray)
V2RAY_BINARY_PATH    = './v2ray'
CLASH_BINARY_PATH    = './clash'

V2RAY_TEST_TIMEOUT   = 3.5    # 延迟测试超时(秒)
REAL_TEST_TIMEOUT    = 6.0    # 真连接测试超时(秒)
REAL_TEST_URL        = 'https://www.google.com/generate_204'

# 并发控制 (本地测试不宜过高，防止端口耗尽或引发本地防护拦截)
...
def safe_request_get(url, proxies=None, timeout=5, allow_redirects=False, headers=None):
    """
    隔离请求环境：
    1. 强制 trust_env=False，防止系统代理/全局VPN污染测速结果
    2. 默认禁止重定向，杜绝 ISP 劫持欺骗
    """
...
def is_modern_protocol(node):
    """判断是否为原版 V2Ray 不支持的现代协议 (需交给 Clash Meta/Mihomo 测试)"""
...
def parse_all_nodes(all_contents):
    ...
    logger.info(f"总计成功解析: {len(all_nodes)} 个唯一节点")
    return all_nodes
...
def test_node_with_v2ray(node):
    # 现代协议原版 V2Ray 不支持，直通交由阶段 1.5 Clash Meta 进行测速
...
def stage1_v2ray_test(nodes):
    logger.info(f"\n{'='*60}")
    logger.info(f"[阶段1/3] 延迟快速初筛 ({len(nodes)} 个节点)")
    logger.info(f"{'='*60}")
    results = []
...
def stage_real_google_test(nodes, pass_num=1):
    ...
    logger.info(f"  目标: {REAL_TEST_URL} (严禁跳转，严格要求 204)")
    logger.info(f"{'='*60}")
...
def verify_scholar_access(socks_address, node_name):
    ...
    try:
        ...
        # 学术搜索可能存在合法的内部重定向，但必须检查是否出现人机验证或封禁关键词
        ...
        if r.status_code == 200:
            ...
            if not blocked:
                logger.info(f"✓ Scholar通过: {node_name}")
                return True
    except: pass
    return False
...
def generate_clash_config(nodes, output_file):
    logger.info(f"生成 Clash 配置文件: {len(nodes)} 个节点 → {output_file}")
    config = {
    ...
    logger.info(f"✓ 写入完成: {output_file}")

def generate_v2ray_base64_subscription(nodes, output_file):
    logger.info(f"生成 V2Ray 订阅: {len(nodes)} 个节点 → {output_file}")
    ...
    with open(plain, 'w', encoding='utf-8') as f:
        f.write(content)
    logger.info(f"✓ 写入完成: {output_file} | 明文: {plain}")
...
def check_environment():
    ...
    try:
        ...
        if r.status_code == 204:
            print("\n" + "!" * 75)
            logger.warning("【特别警告】检测到当前测试机【在没有通过代理的情况下可直连 Google】！")
            logger.warning("原因排查：")
            logger.warning("  1. 脚本正运行在【境外服务器 / VPS / GitHub Actions】上；")
            logger.warning("  2. 本机开启了 TUN / 网卡级全局透明代理。")
            logger.warning("后果：测试出的可用节点未经过 GFW 过滤，在境内电脑上导入将【完全无法使用】！")
            logger.warning("解决：请务必在【境内无全局代理的本机网络环境】下直接运行本脚本。")
            print("!" * 75 + "\n")
            time.sleep(3)
    except:
        logger.info("✓ 网络环境检测正常：本地网络被 GFW 拦截，符合境内测试条件。")

# ==================== 主流程 ====================
def main():
    logger.info("="*60)
    logger.info("Clash/V2Ray 节点深度过滤与聚合工具 v2.2")
    logger.info("流程: 延迟初筛 → 真连接Google严格校验(×2) → Scholar学术验证")
    logger.info("="*60)
    ...
    if not os.path.exists(CLASH_BINARY_PATH):
        ...
        logger.error("重要提醒: 请下载 Mihomo (Clash Meta) 内核并重命名为 clash 放至本目录。")
        logger.info("下载地址: https://github.com/MetaCubeX/mihomo/releases")
        sys.exit(1)
    ...
    logger.info(f"订阅链接数: {len(urls)}，开始抓取...")
    contents = {}
    ...
    if not contents:
        logger.error("所有订阅均抓取失败，请检查网络或链接有效性"); return

    # 解析节点
    all_nodes = parse_all_nodes(contents)
    if not all_nodes:
        logger.warning("未解析出任何节点，程序退出"); return
    ...
    if not real1_nodes:
        logger.warning("真连接测试第1轮无节点通过"); return

    # ── 阶段1.7: 真连接Google第2轮 (剔除偶发不稳定节点) ──
    real2_nodes = stage_real_google_test(real1_nodes, pass_num=2) if STAGE15_REAL_TEST else real1_nodes
    if not real2_nodes:
        logger.warning("真连接测试第2轮无节点通过"); return
    ...
    with open(UPDATE_TIME_FILE, 'w', encoding='utf-8') as f:
        ...
        f.write(f"总解析节点数: {len(all_nodes)}\n")
